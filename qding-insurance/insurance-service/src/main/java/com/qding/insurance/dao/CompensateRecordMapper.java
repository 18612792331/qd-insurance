...
public interface CompensateRecordMapper {
	 ...
	 * 根据单证ID查询理赔记录
	 */
	List<CompensateRecordResult> selectPolicyCompensate(@Param("policyId")String policyId,@Param("guaranteeItemId")String guaranteeItemId);
	/**
	 * 理赔记录数总数
	 */
	 ...
	 * 根据单证ID查询用户保单理赔列表
	 */
	List<CompensateRecordVo> selectCompensateRecordListByPolicyIDandStatus(
			@Param("policyID")String policyID, @Param("status")Integer status);
	/**
	 * 
	 * 根据房屋ID和单证ID查理赔记录
	 */
	CompensateRecord selectByPolicyIdAndRoomId(@Param("roomId")String roomId,@Param("policyID")String policyId);
	
	/**
	 * 
	 * 根据房屋ID和单证ID查理赔记录
	 */
	CompensateRecord selectByOrderCode(@Param("roomId")String roomId,@Param("orderCode")String orderCode);

	CompensateRecordVo selectById(String id);
}